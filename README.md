...
- **JavaScript rendering is optional but supported.** GitHub Actions installs Chromium and the audit can render a bounded sample of pages. Rendered DOM evidence is used for content, accessibility, SEO, templates and interaction signals; pages that remain loading shells are excluded from those scores.
